...
apply_patch "build/make" "../../$PATCH_DIR/build_make_ota_spl_downgrade_variable.patch" "build-ota-package-target: pass --spl_downgrade to ota_from_target_files.py when TARGET_OTA_ALLOW_SPL_DOWNGRADE is true, so SPL_DOWNGRADE=1 lands in payload_properties.txt and update_engine's CheckSPLDowngrade() is skipped on the installing device regardless of which build produced it"
