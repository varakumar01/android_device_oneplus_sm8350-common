...
apply_patch "system/update_engine" "../../$PATCH_DIR/system_update_engine_drop_spl_downgrade_check.patch" "DeltaPerformer::CheckSPLDowngrade(): drop the SPL-downgrade reject/powerwash gate -- AxionOS never ships a genuine SPL downgrade, and ro.build.version.security_patch/ro.boot.verifiedbootstate are not trustworthy on a device running root/integrity-spoofing tooling, which can turn a normal update into a false-positive hard reject or forced wipe"
