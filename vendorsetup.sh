...
apply_patch "packages/apps/Updater" "../../../$PATCH_DIR/packages_apps_updater_purge_uninstallable_on_restore.patch" "UpdaterController: drop DB rows for updates Utils.canInstall() rejects (already installed / older than the running build) at startup instead of leaving a dead Resume card behind after an OTA"
